...
local function setupMythicShopItems(id, shopType, shopName, groups)
	local shop = id and MShops[shopType][id] or MShops[shopType] --[[@as OxShop]]
...
local function registerMythicShopType(shopType, properties)
	local shopLocations = properties.tlocations

	if shopLocations and type(shopLocations) == "table" then
		MShops[shopType] = {
			label = properties.name,
			id = shopType,
			items = properties.inventory,
			slots = #properties.inventory,
			type = "shop",
			tlocations = shopLocations,
		}
	end

	setupMythicShopItems(nil, shopType, properties.name, properties.groups or properties.jobs)
end

for shopType, shopDetails in pairs(lib.load('data.mythicShops') or {}) do
	registerMythicShopType(shopType, shopDetails)
end
...
lib.callback.register('ox_inventory:openMythicShop', function(source, data)
	local left, shop = Inventory(source)
	...
	if data then
		...
		if shop.tlocations and type(shop.tlocations) == "table" then
			for _, loc in ipairs(shop.tlocations) do
				local coords = loc.coords
				if coords and #coords >= 3 then
					if #(pedCoords - vector3(coords[1], coords[2], coords[3])) <= 10.0 then
						valid = true
						break
					end
				end
...
lib.callback.register('ox_inventory:buyItem', function(source, data)
	if data.toType == 'player' then
		...
		if not shop then
			shop = shopId and MShops[shopType][shopId] or MShops[shopType]
		end
