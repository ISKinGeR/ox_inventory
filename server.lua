...
local MythicInvTypes = lib.load('data.mythicInvType')
...
local function openInventory(source, invType, data, ignoreSecurityChecks)
	...
	if data then
		...
		elseif isDataTable and not invType == 'OldMythic' then
			if data.netid then
		...
		elseif invType == 'OldMythic' then
				right = Inventory(data.id)
			
				if not right then
					local invTypeData
			
					for _, v in pairs(MythicInvTypes) do
						if v.id == data.inventory.invType then
							invTypeData = v
							break
						end
					end
					...
					if invTypeData then			
						right = Inventory.Create(
							data.id,
							invTypeData.name,
							"stash",
							invTypeData.slots or 50,
							0,
							invTypeData.capacity or 100000,
							false
						)
					end
				end
...
lib.callback.register('ox_inventory:openOldMythic', function(source, data)
	...
	if data then
		...
		-- if data.business --[[or data.]] then
		-- 	if server.CheckJob(source,data) then

		-- 	end
		-- end
		local KR, IKR = openInventory(source, "OldMythic", data)
		return KR, IKR
	end
	return
end)
