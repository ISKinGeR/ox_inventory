...
function client.openInventory(inv, data)
	...
	elseif inv == 'MythicShop' then
		if cache.vehicle then
            return lib.notify({ id = 'cannot_perform', type = 'error', description = locale('cannot_perform') })
        end
		left, right, accessError = lib.callback.await('ox_inventory:openMythicShop', 200, data)
	...
	if _CacheMythic._inInvPoly ~= nil then
		if cache.vehicle then
            return lib.notify({ id = 'cannot_perform', type = 'error', description = locale('cannot_perform') })
        end

		left, right, accessError = lib.callback.await('ox_inventory:openOldMythic', 200, _CacheMythic._inInvPoly)
	end
